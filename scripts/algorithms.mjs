...
const computeHTable = (nodes, finalNodes, heuristic) => {
  return nodes.map((node) => heuristic(node, finalNodes));
};
...
export const bestFirstSearch =
  (heuristic) => (nodes, matrix, originIdx, onDiscover, onVisit, onFind) => {
    const finalNodes = nodes.filter((node) => node.type === NODE_TYPE.FINAL);
    const discovered = new Array(nodes.length).fill(0);
    const order = new Array(nodes.length).fill(-1);
    const HTable = computeHTable(nodes, finalNodes, heuristic);
